...
class TrackFetcher(webapp.RequestHandler):
        ...
        for i in range(maxtokens):
                ...
                if month is None:
                    ...
                    if match is not None:
                        ...
                        if len(matchlist) < 3:
                            ...
                            y = matchlist[2]
                        print >> sys.stderr, "[%s] %s" % (matchlist, line)
                        thedate = date(int(y), int(matchlist[1]), int(matchlist[0]))
                        break
                else: # try matching long date format
                    if day is None:
            ...
            if thedate is None:
                if day is not None and month is not None:
                    print >> sys.stderr, "%d/%d/%d [%s]" % (day, month, year, line)
    ...
    #for item in matchlist:
        if match is not None:
            ...
            if len(artist) == 0:
                tmp = title.split("/")
                if len(tmp) < 2: 
                    print >> sys.stderr, "unable to parse artist from title: %s" % (title)
                    return None
                title = tmp[0].strip(' ')
                artist = tmp[1].strip(' ')
            ...
            #print "[%d] %s - %s" % (self._total_parsed, artist, title)
                print >> sys.stderr, "[%d] %s - %s" % (self._total_parsed, t.artist, t.title)
    ...
    def ParseProgramTracks(self, programid):
        ...
        if _result.status_code != 200:
            print "No Program matches id=" + str(programid)
            return
            ...
            if _program is None:
                ...
                if len(matchlist) > 2:
                    ...
                    if _program is None:
                        ...
                        _program.put()
                    print >> sys.stderr, "[%d] Program='%s' host='%s' timeslot='%s'" % (programid, _program.name,_program.host,_program.timeslot)
                else:
                    pass
                    #print "Trouble matching hdr with id=" +str(programid)
                continue
    ...
    def ParseProgramIndex(self, mainurl):
        ...
        if result.status_code != 200:
            return
        hnow = datetime.now(ozdst())
        print hnow.strftime("%H %M")
        hour=-2
        day=0
        for item in result.content.splitlines():
            ...
            if match is not None:
                ...
                #proglist.append((int(program),) )
                #print "%d %s %d" % (int(program), Weekdays[day-1], hour)
                if hnow.hour - hour >= 3 and hnow.hour - hour <= 6:
                    programs.append(int(program))
        #print programs
        #return 
        #####programs = progmatcher.findall(result.content)
        #histogram = {}
        for i in programs[3:4]:
            self.ParseProgramTracks(111)
